...
class Module(core.module.Module):
        ...
        try:
            ...
            for widget in self.widgets():
                if widget.name == "playerctl.pause":
                    playback_status = str(util.cli.execute(
                        self.__cmd + "status")).strip()
                    if playback_status != "":
    ...
    def __get_song(self):
        album = str(util.cli.execute(
            self.__cmd + "metadata xesam:album")).strip()
        title = str(util.cli.execute(
            self.__cmd + "metadata xesam:title")).strip()
        artist = str(util.cli.execute(
            self.__cmd + "metadata xesam:albumArtist")).strip()
        track_number = str(util.cli.execute(
            self.__cmd + "metadata xesam:trackNumber")).strip()
    ...
    def state(self, widget):
        ...
        if playback_status != "":
            return self.threshold_state(60, 50, 90)
        else:
            return self.threshold_state(40, 50, 90)
